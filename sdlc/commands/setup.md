...
The current plugin version is **7.0.0**. Compare what you found in the config:

**If the config shows `sdlc_version: "7.0.0"` (versions match):**
```
SDLC is already configured and up to date (v7.0.0).
...
- Worktree coordination (prevents conflicts between parallel Claude instances)
- Session task tracking (clear "what's next?" visibility using TaskTools)
- On-demand TDD cycle task creation (focused task lists)
...
4. **For v6.0.0 → v7.0.0 migration**: Ask about new features if `features` section is missing:
   - Worktree coordination (if git.worktrees is true)
...
8. Update `sdlc_version` in `.claude/sdlc.yaml` to **7.0.0**
...
**Question 2c: Enable Worktree Coordination? (v7.0.0)**
...
**Question 2d: Enable Session Task Tracking? (v7.0.0)**
