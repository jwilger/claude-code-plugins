...
The current plugin version is **3.12.3**. Compare what you found in the config:

**If the config shows `sdlc_version: "3.12.3"` (versions match):**
```
SDLC is already configured and up to date (v3.12.3).
...
6. Update `sdlc_version` in `.claude/sdlc.yaml` to **3.12.3**
...
- Add .claude/sdlc.yaml with project preferences and language patterns (v3.12.3)
...
### Changes
- Created \`.claude/sdlc.yaml\` with project preferences and language-specific patterns (v3.12.3)
...
# If PR workflow is enabled:
git checkout -b sdlc-update-v3.12.3
git commit -m "chore: update SDLC configuration to v3.12.3

- Update .claude/sdlc.yaml (v<old> → v3.12.3)
...
git push -u origin sdlc-update-v3.12.3
gh pr create --title "chore: update SDLC configuration to v3.12.3" --body "## Summary

This PR updates the SDLC workflow configuration from v<old> to v3.12.3.
...
```
✅ SDLC initialized successfully! (v3.12.3)
...
```
✅ SDLC updated successfully! (v<old> → v3.12.3)
