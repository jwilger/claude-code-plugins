...
The current plugin version is **3.12.2**. Compare what you found in the config:

**If the config shows `sdlc_version: "3.12.2"` (versions match):**
```
SDLC is already configured and up to date (v3.12.2).
...
6. Update `sdlc_version` in `.claude/sdlc.yaml` to **3.12.2**
...
### 7. Prepare TDD Hooks Structure

Based on the language configuration gathered in Step 5, prepare the hooks configuration structure that will be embedded in `.claude/settings.json` (Step 8).
  ...
  "SessionStart": [
    {
      "hooks": [
        {
          "type": "prompt",
          "prompt": "🧠 MEMORY PROTOCOL REMINDER\n\nThis is a new session. Before diving into work, consider checking memento for relevant context.\n\nMemento may have useful information about:\n- 🐛 Debugging - Similar issues solved before?\n- 🏗️ Architecture - Patterns or decisions documented?\n- 🔧 Tools - Known quirks, workarounds, or configurations?\n- 📋 Conventions - Project-specific patterns or preferences?\n\nTo search: Use mcp__memento__semantic_search with your task description as the query.\n\nThis is a GENTLE REMINDER, not a requirement. If you're confident the work is novel or memento isn't available, proceed without searching. This reminder does not require a response - proceed directly with the user's request."
        }
      ]
    }
  ],
...
**Important**: Do NOT write this as a separate file. This structure will be embedded into `.claude/settings.json` in Step 8 below. Prepare this structure in memory or as a variable for merging in the next step.

### 8. Configure Settings with Embedded Hooks
...
Create or update `.claude/settings.json` to include the output style, hooks configuration (from Step 7), and recommended settings. Preserve any existing settings (like permissions) and add/update:
...
```json
{
  ...
  "hooks": {
    ...
    "SubagentStop": [/* hooks from Step 7 */],
    "PreCompact": [/* hook from Step 7 */],
    "Stop": [/* hook from Step 7 */]
  }
}
```
...
```json
{
  ...
  "hooks": {
    /* ...hooks configuration from Step 7... */
  }
}
```
...
2. Embed the complete hooks structure from Step 7 under the `hooks` key
...
- Add .claude/sdlc.yaml with project preferences and language patterns (v3.12.2)
...
### Changes
- Created \`.claude/sdlc.yaml\` with project preferences and language-specific patterns (v3.12.2)
...
# If PR workflow is enabled:
git checkout -b sdlc-update-v3.12.2
git commit -m "chore: update SDLC configuration to v3.12.2

- Update .claude/sdlc.yaml (v<old> → v3.12.2)
- Move hooks from .claude/hooks.json into .claude/settings.json (CRITICAL FIX)
- Remove legacy .claude/hooks.json file
- <List any new features or changes in this version>"
git push -u origin sdlc-update-v3.12.2
gh pr create --title "chore: update SDLC configuration to v3.12.2" --body "## Summary

This PR updates the SDLC workflow configuration from v<old> to v3.12.2.

### Changes
- Updated \`.claude/sdlc.yaml\` version field
- **CRITICAL FIX**: Moved hooks from \`.claude/hooks.json\` into \`.claude/settings.json\`
  - Claude Code only reads project-level hooks from settings.json, not from separate hooks.json files
  - This fixes TDD enforcement which was completely non-functional in previous versions
...
```
✅ SDLC initialized successfully! (v3.12.2)
...
```
✅ SDLC updated successfully! (v<old> → v3.12.2)
