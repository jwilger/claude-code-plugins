...
The current plugin version is **16.0.0**. Compare what you found in the config:

**If the config shows `sdlc_version: "16.0.0"` (versions match):**
```
SDLC is already configured and up to date (v16.0.0).
...
6. Update `sdlc_version` in `.claude/sdlc.yaml` to **16.0.0**
...
**Note**: Previous versions used `gh-issue-ext` and `gh-project-ext` for GitHub Issues/Projects integration. These are no longer needed as v16.0.0 uses `dot` CLI for local task management.
...
**Note on GitHub Integration**: Previous versions (v4.x) used GitHub Issues and Projects for task management. Version 16.0.0 uses local `dot` CLI for faster, offline-capable task management. GitHub integration is now limited to pull requests and code review only.
...
# If PR workflow is enabled:
git checkout -b sdlc-update-v16.0.0
git commit -m "chore: update SDLC configuration to v16.0.0

- Update .claude/sdlc.yaml (v<old> → v16.0.0)
...
git push -u origin sdlc-update-v16.0.0
gh pr create --title "chore: update SDLC configuration to v16.0.0" --body "## Summary

This PR updates the SDLC workflow configuration from v<old> to v16.0.0.
...
```
✅ SDLC initialized successfully! (v16.0.0)
...
```
✅ SDLC updated successfully! (v<old> → v16.0.0)
