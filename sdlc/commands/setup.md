...
The current plugin version is **3.12.4**. Compare what you found in the config:

**If the config shows `sdlc_version: "3.12.4"` (versions match):**
```
SDLC is already configured and up to date (v3.12.4).
...
6. Update `sdlc_version` in `.claude/sdlc.yaml` to **3.12.4**
...
- Add .claude/sdlc.yaml with project preferences and language patterns (v3.12.4)
...
### Changes
- Created \`.claude/sdlc.yaml\` with project preferences and language-specific patterns (v3.12.4)
...
# If PR workflow is enabled:
git checkout -b sdlc-update-v3.12.4
git commit -m "chore: update SDLC configuration to v3.12.4

- Update .claude/sdlc.yaml (v<old> → v3.12.4)
...
git push -u origin sdlc-update-v3.12.4
gh pr create --title "chore: update SDLC configuration to v3.12.4" --body "## Summary

This PR updates the SDLC workflow configuration from v<old> to v3.12.4.
...
```
✅ SDLC initialized successfully! (v3.12.4)
...
```
✅ SDLC updated successfully! (v<old> → v3.12.4)
