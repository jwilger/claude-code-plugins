...
The current plugin version is **19.1.0**. Compare what you found in the config:

**If the config shows `sdlc_version: "19.1.0"` (versions match):**
```
SDLC is already configured and up to date (v19.1.0).
...
8. Update `sdlc_version` in `.claude/sdlc.yaml` to **19.1.0**
...
**Note**: Previous versions used `gh-issue-ext` and `gh-project-ext` for GitHub Issues/Projects integration. These are no longer needed as v19.1.0 uses `dot` CLI for local task management.
...
**Note on GitHub Integration**: Previous versions (v4.x) used GitHub Issues and Projects for task management. Version 19.1.0 uses local `dot` CLI for faster, offline-capable task management. GitHub integration is now limited to pull requests and code review only.
...
- Add .claude/sdlc.yaml with project preferences and language patterns (v19.1.0)
...
### Changes
- Created \`.claude/sdlc.yaml\` with project preferences and language-specific patterns (v19.1.0)
...
# If PR workflow is enabled:
git checkout -b sdlc-update-v19.1.0
git commit -m "chore: update SDLC configuration to v19.1.0

- Update .claude/sdlc.yaml (v<old> → v19.1.0)
...
git push -u origin sdlc-update-v19.1.0
gh pr create --title "chore: update SDLC configuration to v19.1.0" --body "## Summary

This PR updates the SDLC workflow configuration from v<old> to v19.1.0.
...
```
SDLC initialized successfully! (v19.1.0)
...
```
SDLC updated successfully! (v<old> → v19.1.0)
