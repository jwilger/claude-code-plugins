...
The current plugin version is **8.0.0**. Compare what you found in the config:

**If the config shows `sdlc_version: "8.0.0"` (versions match):**
```
SDLC is already configured and up to date (v8.0.0).
...
- **BREAKING**: Commit-based ADRs (decision context in commit messages, not separate files)
- Architecture isolation enforcement via git hooks (pre-commit framework)
- `/arch` skill for architecture changes (portable across all AI coding assistants)
- Automatic migration of existing ADR files to docs/adr-archive/
...
4. **For v7.0.0 → v8.0.0 migration**: Run migration for commit-based ADRs:
   - Detect existing docs/adr/*.md files
   - Run sdlc/scripts/migrate-to-v8.sh
   - Install pre-commit framework and hooks
5. Create `.dots/.worktrees/` directory if worktree coordination enabled
6. Regenerate `.claude/settings.json` with hooks embedded and new templates
7. Remove legacy `.claude/hooks.json` file if it exists (hooks are now in settings.json)
8. Update `sdlc_version` in `.claude/sdlc.yaml` to **8.0.0**
...
**Note**: Previous versions used `gh-issue-ext` and `gh-project-ext` for GitHub Issues/Projects integration. These are no longer needed as v6.0.0 uses `dot` CLI for local task management.
...
**Question 2c: Enable Worktree Coordination? (v8.0.0)**

**Only ask if worktrees are enabled.** If worktrees are disabled, skip this question and set `worktree_coordination: false`.

If worktrees are enabled, ask:
- Yes (Recommended) - Prevent conflicts when multiple Claude instances work on different tasks
- No - Skip coordination (allows conflicts, not recommended for parallel work)

Explain:
- Coordination tracks which tasks are being worked on in which worktrees
- Prevents two Claude instances from working on the same task simultaneously
- Warns if you try to start a task while its blocker is in progress elsewhere
- Automatically cleans up stale registrations (> 2 hours old)

**Question 2d: Enable Session Task Tracking? (v8.0.0)**

Ask:
- Yes (Recommended) - Show "what's next?" using Claude Code's TaskTools
- No - Skip task tracking (manual workflow management)

Explain:
- Creates a story-level task when starting work on an issue
- Shows TDD cycle progress (Red → Domain → Green → Domain)
- Provides clear "what's next?" visibility after each phase
- Keeps task list focused by creating TDD tasks on-demand
...
# Create worktree coordination directory if worktree_coordination enabled
if [ "$WORKTREE_COORDINATION" = "true" ]; then
  mkdir -p .dots/.worktrees
  echo "✓ Worktree coordination directory created"
fi
```

**Note on GitHub Integration**: Previous versions (v4.x) used GitHub Issues and Projects for task management. Version 6.0.0 uses local `dot` CLI for faster, offline-capable task management. GitHub integration is now limited to pull requests and code review only.
...
# Feature flags (v8.0.0+)
features:
  worktree_coordination: true    # Prevent conflicts between parallel Claude instances
  session_task_tracking: true    # Enable TaskTools for "what's next?" visibility
...
# If PR workflow is enabled:
git checkout -b sdlc-update-v6.0.0
git commit -m "chore: update SDLC configuration to v6.0.0

- Update .claude/sdlc.yaml (v<old> → v6.0.0)
...
git push -u origin sdlc-update-v6.0.0
gh pr create --title "chore: update SDLC configuration to v6.0.0" --body "## Summary

This PR updates the SDLC workflow configuration from v<old> to v6.0.0.
...
### 11a. Install Pre-Commit Hooks (v8.0.0+)

**IMPORTANT:** v8.0.0 introduces architecture isolation enforcement via git hooks.
...
if ! command -v pre-commit &> /dev/null; then
  ...
  echo ""
  echo "Pre-commit is required for architecture isolation hooks (v8.0.0+)."
  echo "Install via:"
  echo "  pip install pre-commit"
  echo "  OR: brew install pre-commit (macOS)"
  echo "  OR: See https://pre-commit.com/#install"
  echo ""
  read -p "Press Enter after installing pre-commit..."
fi
```

Copy pre-commit configuration and hook scripts:

```bash
# Copy .pre-commit-config.yaml to project root
if [[ ! -f .pre-commit-config.yaml ]]; then
  cp "${CLAUDE_PLUGIN_ROOT}/sdlc/templates/.pre-commit-config.yaml" .
  echo "✅ Copied .pre-commit-config.yaml"
else
  echo "ℹ️  .pre-commit-config.yaml already exists (not overwriting)"
fi

# Copy hook scripts to sdlc/hooks/ (if they don't exist)
mkdir -p sdlc/hooks
for hook in check-architecture-isolation.sh check-adr-format.sh check-branch-consistency.sh; do
  if [[ ! -f "sdlc/hooks/$hook" ]]; then
    cp "${CLAUDE_PLUGIN_ROOT}/sdlc/hooks/$hook" sdlc/hooks/
    chmod +x "sdlc/hooks/$hook"
    echo "✅ Installed sdlc/hooks/$hook"
  fi
done
...
### 11b. Optional CI Workflow (v8.0.0+)

Ask user if they want to install the GitHub Actions workflow for architecture validation:

**Use AskUserQuestion:**
- **Question**: "Install GitHub Actions workflow for architecture validation?"
- **Header**: "CI Workflow"
- **Options**:
  - "Yes" - Adds .github/workflows/architecture-validation.yml (validates architecture isolation in PRs)
  - "No" - Skip CI workflow (can add later manually)

**If user chooses "Yes":**

```bash
mkdir -p .github/workflows
cp "${CLAUDE_PLUGIN_ROOT}/sdlc/templates/workflows/architecture-validation.yml" .github/workflows/
echo "✅ Installed .github/workflows/architecture-validation.yml"
echo ""
echo "This workflow validates architecture commits in PRs:"
echo "  - Enforces commit isolation (ARCHITECTURE.md only)"
echo "  - Validates ADR format in commit messages"
echo "  - Checks branch consistency"
```

### 11c. Migrate Existing ADRs (v8.0.0 Migration)

**Only run this for v7.0.0 → v8.0.0 upgrades (not fresh installs).**

Check if docs/adr/ exists with ADR files:

```bash
if [[ -d "docs/adr" ]] && [[ $(find docs/adr -name "*.md" -type f 2>/dev/null | wc -l) -gt 0 ]]; then
  echo ""
  echo "📦 Existing ADR files detected"
  echo ""
  echo "v8.0.0 uses commit-based ADRs (decision context in commit messages)."
  echo "Your existing ADR files will be archived to docs/adr-archive/."
  echo ""

  bash "${CLAUDE_PLUGIN_ROOT}/sdlc/scripts/migrate-to-v8.sh"
fi
```
...
```
✅ SDLC initialized successfully! (v6.0.0)
...
```
✅ SDLC updated successfully! (v<old> → v6.0.0)
