...
Compare the version in the config to the current plugin version (**3.10.1**).

**If versions match:**
```
SDLC is already configured and up to date (v3.10.1).
...
6. Update `sdlc_version` in `.claude/sdlc.yaml` to **3.10.1**
...
- Add .claude/sdlc.yaml with project preferences and language patterns (v3.10.1)
...
### Changes
- Created \`.claude/sdlc.yaml\` with project preferences and language-specific patterns (v3.10.1)
...
# If PR workflow is enabled:
git checkout -b sdlc-update-v3.10.1
git commit -m "chore: update SDLC configuration to v3.10.1

- Update .claude/sdlc.yaml (v<old> → v3.10.1)
...
git push -u origin sdlc-update-v3.10.1
gh pr create --title "chore: update SDLC configuration to v3.10.1" --body "## Summary

This PR updates the SDLC workflow configuration from v<old> to v3.10.1.
...
```
✅ SDLC initialized successfully! (v3.10.1)
...
```
✅ SDLC updated successfully! (v<old> → v3.10.1)
