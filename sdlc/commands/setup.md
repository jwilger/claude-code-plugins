...
The current plugin version is **17.0.1**. Compare what you found in the config:

**If the config shows `sdlc_version: "17.0.1"` (versions match):**
```
SDLC is already configured and up to date (v17.0.1).
...
6. Update `sdlc_version` in `.claude/sdlc.yaml` to **17.0.1**
...
**Note**: Previous versions used `gh-issue-ext` and `gh-project-ext` for GitHub Issues/Projects integration. These are no longer needed as v17.0.1 uses `dot` CLI for local task management.
...
**Note on GitHub Integration**: Previous versions (v4.x) used GitHub Issues and Projects for task management. Version 17.0.1 uses local `dot` CLI for faster, offline-capable task management. GitHub integration is now limited to pull requests and code review only.
...
# If PR workflow is enabled:
git checkout -b sdlc-update-v17.0.1
git commit -m "chore: update SDLC configuration to v17.0.1

- Update .claude/sdlc.yaml (v<old> → v17.0.1)
...
git push -u origin sdlc-update-v17.0.1
gh pr create --title "chore: update SDLC configuration to v17.0.1" --body "## Summary

This PR updates the SDLC workflow configuration from v<old> to v17.0.1.
...
```
✅ SDLC initialized successfully! (v17.0.1)
...
```
✅ SDLC updated successfully! (v<old> → v17.0.1)
