...
The current plugin version is **3.12.7**. Compare what you found in the config:

**If the config shows `sdlc_version: "3.12.7"` (versions match):**
```
SDLC is already configured and up to date (v3.12.7).
...
6. Update `sdlc_version` in `.claude/sdlc.yaml` to **3.12.7**
...
- Add .claude/sdlc.yaml with project preferences and language patterns (v3.12.7)
...
### Changes
- Created \`.claude/sdlc.yaml\` with project preferences and language-specific patterns (v3.12.7)
...
# If PR workflow is enabled:
git checkout -b sdlc-update-v3.12.7
git commit -m "chore: update SDLC configuration to v3.12.7

- Update .claude/sdlc.yaml (v<old> → v3.12.7)
...
git push -u origin sdlc-update-v3.12.7
gh pr create --title "chore: update SDLC configuration to v3.12.7" --body "## Summary

This PR updates the SDLC workflow configuration from v<old> to v3.12.7.
...
```
✅ SDLC initialized successfully! (v3.12.7)
...
```
✅ SDLC updated successfully! (v<old> → v3.12.7)
